...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"SSMBProgressHUD needs to be accessed on the main thread.");

CGFloat const SSMBProgressMaxOffset = 1000000.f;
...
@interface SSMBProgressHUDRoundedButton : UIButton
@end


@implementation SSMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    SSMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    SSMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (SSMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (SSMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = SSMBProgressHUDAnimationFade;
    _mode = SSMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(SSMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == SSMBProgressHUDAnimationZoom) {
        type = animatingIn ? SSMBProgressHUDAnimationZoomIn : SSMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == SSMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == SSMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == SSMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == SSMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    SSMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<SSMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    SSMBBackgroundView *backgroundView = [[SSMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = SSMBProgressHUDBackgroundStyleSolidColor;
    ...
    SSMBBackgroundView *bezelView = [SSMBBackgroundView new];
    ...
    UIButton *button = [SSMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[SSMBRoundProgressView class]];

    SSMBProgressHUDMode mode = self.mode;
    if (mode == SSMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == SSMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[SSMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SSMBProgressHUDModeDeterminate || mode == SSMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[SSMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == SSMBProgressHUDModeAnnularDeterminate) {
            [(SSMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == SSMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == SSMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[SSMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[SSMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[SSMBRoundProgressView class]]) {
        SSMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SSMBRoundProgressView appearanceWhenContainedIn:[SSMBProgressHUD class], nil];
#else
        appearance = [SSMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[SSMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((SSMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((SSMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[SSMBBarProgressView class]]) {
        SSMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [SSMBBarProgressView appearanceWhenContainedIn:[SSMBProgressHUD class], nil];
#else
        appearance = [SSMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[SSMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((SSMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((SSMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    SSMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(SSMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == SSMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation SSMBRoundProgressView
...
@implementation SSMBBarProgressView
...
@implementation SSMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = SSMBProgressHUDBackgroundStyleBlur;
            ...
            _style = SSMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(SSMBProgressHUDBackgroundStyle)style {
    if (style == SSMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = SSMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    SSMBProgressHUDBackgroundStyle style = self.style;
    if (style == SSMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == SSMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation SSMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [SSMBProgressHUD allHUDsForView:view];
    for (SSMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable SSMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    SSMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == SSMBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = SSMBProgressHUDBackgroundStyleSolidColor;
...
@implementation SSMBProgressHUDRoundedButton
